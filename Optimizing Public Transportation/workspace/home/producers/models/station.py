...
class Station(Producer):
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        logger.info(f"Producing arrival event for train {train.train_id} at station {self.station_id}")
    ...
    def __str__(self):
        return f"Station | {self.station_id:^5} | {self.name:<30} | Direction A: | {self.a_train.train_id if self.a_train is not None else '---':^5} | departing to {self.dir_a.name if self.dir_a is not None else '---':<30} | Direction B: | {self.b_train.train_id if self.b_train is not None else '---':^5} | departing to {self.dir_b.name if self.dir_b is not None else '---':<30} | "
