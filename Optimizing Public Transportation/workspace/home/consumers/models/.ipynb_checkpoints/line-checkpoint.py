...
class Line:
    ...
    def __init__(self, color):
        """Creates a line with a specific color and initializes stations"""
        self.color = color
    ...
    def _handle_station(self, value):
        """Adds the station to this Line's data model if it matches the line color"""
        if value["line"] != self.color:
            return
        self.stations[value["station_id"]] = Station.from_message(value)

    def _handle_arrival(self, message):
        """Updates train locations based on the arrival message"""
        value = message.value()
    ...
    def process_message(self, message):
        """Processes a Kafka message by calling the appropriate handler based on the topic"""
        topic = message.topic()

        if topic == "stations_topic":  # Set to your stations Faust Table topic
            try:
        ...
        elif topic == "arrivals_topic":  # Set to your arrivals topic
            self._handle_arrival(message)
        
        elif topic == "turnstile_summary_topic":  # Set to your KSQL Turnstile Summary Topic
